...
if (!empty($_POST)) {
    ...
    if ($verif == false) {
    ...
    } else {  // on récupére les valeurs de nos champs et on les stocke dans des variables
        ...
        $month = (date('m'));
        $date = (date('d'));
        $dateLimitSup = $year1 . "-" . $month . "-" . $date; // date limite supérieure      

        $year2 = ((int) date('Y')) - 90;
        $dateLimitInf = $year2 . "-" . $month . "-" . $date;  //date limite inférieure
    ...
    if (!isset($city) || strlen($city) > 50 || preg_match($regex, $city)) {
        ...
        if ($emailExist && $pseudoExist) { // vérifier si le pseudo existe dans la BDD
            $info = alert("Vous avez déjà un compte", "danger");
        } else if (empty($info)) {

            $mdpHash = password_hash($mdp, PASSWORD_DEFAULT);

            inscriptionUsers($lastName,  $firstName,  $pseudo,  $email,  $phone,  $mdpHash,  $civility,  $birthday, $city,);
...
<main style="background:url(assets/img/logoviet2.jpg) no-repeat; background-size:35%; background-attachment: fixed; background-position: center">
    ...
    <div class="w-75 m-auto p-5" style="background: rgba(20, 20, 20, 0.5);">
        ...
        <form action="" method="post" class="p-5">
            ...
            <div class="row mb-3">
                ...
                <div class=" mb-5">
                    <label for="email" class="form-label mb-3">Email ou Email du responsable légale</label>
