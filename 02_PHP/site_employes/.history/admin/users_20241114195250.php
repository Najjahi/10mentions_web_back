...
if (isset($_GET) && isset($_GET['action']) && isset($_GET['id_user'])) {
    ...
    if ($_GET['action'] == 'update' && !empty($_GET['id_user'])) {
        ...
        if (is_numeric($idUser)) { //On vérifie si la valeur soumise pour le champ "id_employe" est bien un nombre.

            $user  = showUserViaId($idUser);
    
            if ($user) {
    
                deleteUser($iduser); //J'appalle la fonction pour supprimer un employe via son ID
            ...
            <table class="table table-dark table-bordered mt-5">
                ...
                <?php
                    ...
                    foreach ($users as $key => $user) {
                        ...
                        $filiale = showFilialeViaId($user['filiale_id']); // on appelle la fonction
