...
if (!empty($_POST)) { //je vérifie si le tableau n'est pas vide, cela veut dire que le formulaire a été envoyé.
    ...
    if ($verif == false || empty($image)) {  // Vérifie s'il y a des champs vides et si l'image n'est téléchargée.
        ...
        $nomEmploye = trim($_POST['nom']);
        $prenomEmploye = trim($_POST['prenom']);
        ...
        if (!isset($nomEmploye) || strlen($nomEmploye) < 2) {
           
            $info .= alert("le nom n'est pas valide ", "danger");
        } 
        if (!isset($prenomEmploye) || strlen($prenomEmploye) < 2) {
        ...
        //on vérifie si l'action dans l'URL est définie, s'il s'agit d'un update, et si l' id_employe existe. 
            ...
            if (isset($_GET) && isset($_GET['action']) && isset($_GET['id_employe']) && !empty($_GET['action']) && 
            !empty($_GET['id_employe']) && $_GET['action'] == 'update') {
                ...
                updateEmployes($image, $nom, $prenom, $matricule, $email, $phone, $ville, $dateEmbauche);
                ...
                if (verifEmploye($nomEmploye, $nomEmploye, $dateEmbauche)) {
                    ...
                    addEmployes($image, $nom, $prenom, $matricule, $email, $phone, $ville, $dateEmbauche);
...
<main>
    ...
    <h2 id="text3d" class="text-center fw-bolder mb-5 text-danger"><?= isset($employe) ? 'MODIFIER UN EMPLOYE' : 'AJOUTER UN EMPLOYE' ?> </h2> 
    ...
    <form action="" method="post" class="back" enctype="multipart/form-data">
        ...
        <div class="row justify-content-center">
            <button type="submit" class="btn btn-danger p-3 w-25"> <?= isset($employe) ? 'MODIFIER UN EMPLOYE' : 'AJOUTER UN EMPLOYE' ?></button>
        </div>
    </form>
</main>
<?php
