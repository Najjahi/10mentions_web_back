...
function createTableUsers()
{
    ...
    $sql = " CREATE TABLE IF NOT EXISTS users (
                ...
                -- ENUM('Celibataire', 'Marié', 'Divorcé', 'Veuf')
...
function foreignKey(string $tableF, string $keyF, string $tableP, string $keyP)
{
    ...
    //$sql ="ALTER TABLE employes ADD FOREIGN KEY (category_id) REFERENCES filiales (id_category)";
    $request = $cnx->exec($sql);
}

//foreignKey('employes', 'category_id', 'filiales', 'id_category');
...
{
    $cnx = connexionBdd();
    $sql = "SELECT * FROM employes WHERE matricule = :matricule AND email = :email";
    $request = $cnx->prepare($sql);
    $request->execute(array(
        ":matricule" => $matricule,
        ":email" => $email
...
function addEmployes(string $image, string $nom, string $prenom, string $matricule, string $email, string $phone, string $ville, string $dateEmbauche)
{
    $cnx = connexionBdd();
    $data = [
        'image' => $image,
    ...
    $sql = "INSERT INTO employes (title, image,  description,  dateCreation,   category_id) VALUES (:title,  :image,  :description,  :dateCreation, :category_id)";
    $request = $cnx->prepare($sql);
    $request->execute(array(
        ':title' => $data['title'],
        ':image' => $data['image'],
...
function verifEmploye(string $titleEmploye, string $dateCreation): mixed
{
    $cnx = connexionBdd();
    $sql = "SELECT * FROM users WHERE title = :title AND dateCreation = :dateCreation";
    $request = $cnx->prepare($sql);
    $request->execute(array(
        ':title' => $titleEmploye,
...
{
    $cnx = connexionBdd();
    $sql = "SELECT * FROM users"; // requête d'insertion que je stock dans une variable
    $request = $cnx->query($sql);
    $result = $request->fetchAll();
    return $result;
}
...
function updateEmployes($image, $titleEmploye, $description, $dateCreation, $dateModification, $id_category, $id_employe): void
{
    $cnx = connexionBdd();
    $data = [
        'image' => $image,
        'titleEmploye' => $titleEmploye,
        'description' => $description,
        'dateCreation' => $dateCreation,
    ...
    $sql = "UPDATE employes SET image =  :image, title = :titleEmploye,  description = :description, dateCreation = :dateCreation,  dateModification = :dateModification, category_id = :category_id WHERE id_employe = :id_employe"; // requête d'insertion que je stock dans une variable

    $request = $cnx->prepare($sql); // je prépare ma fonction et je l'exécute
    $request->execute(array(
        ':image' => $data['image'],
        ':titleEmploye' => $data['titleEmploye'],
        ':description' => $data['description'],
        ':dateCreation' => $data['dateCreation'],
...
function createTableCommentaire()
{
    ...
    $sql = " CREATE TABLE IF NOT EXISTS commentaires (
         ...
         commentaire TEXT,
         CONSTRAINT com_employe FOREIGN KEY (employe_id) REFERENCES employes(id_employe)          
...
function addCommentaire(int $employe_id, string $commentaire, int $id_user)
{

    $cnx = connexionBdd();
    $sql = "INSERT INTO commentaires (commentaire, employe_id, user_id) VALUES (:commentaire, :employe_id, :user_id)";
    $request = $cnx->prepare($sql);
    $request->execute(array(
...
// function updateCommentaire(int $employe_id, string $commentaire):void
// {
//     $cnx = connexionBdd();
//     $sql = "UPDATE commentaires SET commentaire =:commentaire WHERE commentaire_id = :commentaire_id";
//     // $sql = "UPDATE filiales  SET name = :name, description = :description WHERE id_category = :id";
//     $request = $cnx->prepare($sql);
//     $request->execute(array(
//         ':employe_id' => $employe_id,
//         ':commentaire' => $commentaire
...
function getEmployeCommentaires(int $id_employe)
{
    $cnx = connexionBdd();
    $sql = "SELECT * FROM commentaires WHERE employe_id = :employe_id";
    $request = $cnx->prepare($sql);
    $request->execute(array(
