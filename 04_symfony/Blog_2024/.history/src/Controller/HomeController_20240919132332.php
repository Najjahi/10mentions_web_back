...
class HomeController extends AbstractController
{
    ...
    public function index(ArticleRepository $articleRepository, CategorieRepository $categorieRepository): Response// chaque action repon avec une Reponcse
    {
     ...
     records/entities from the database table associated with the `Article` entity. */
        ...
        $catergories = $categorieRepository->findAll();// un tableau avec tout les articles
        // dd($catergories);       
        ...
        return $this->render('home/accueil.html.twig', [// render prends 2 arguments : 
            ...
            "articlesBlog" => $articles, // ici in il inject une variable Qui s'appelle articlesBlog avec sa valeur : le chaine de canractére HomeControler
            "categoriesMenu" => $catergories,
        ]);
    }
    #[Route('/category/{name}', name: 'app_article_category')]
    public function selectCategorie(ArticleRepository $articleRepository, CategorieRepository $categorieRepository, string $name) :Response{
       
        $catergorie = $categorieRepository->findByName($name) ;
        $IdCat = $categorie ->getId();
        $articles = $articleRepository->findByExampleField($IdCat);
         
         //pour la barre de navigation
           
        
        $catergorie = $categorieRepository->findAll();  
